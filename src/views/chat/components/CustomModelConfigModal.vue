    ...
    <div class="chat-card rounded-2xl sm:rounded-3xl max-w-full sm:max-w-4xl w-full overflow-hidden flex flex-col shadow-2xl border border-theme-border" style="max-height: min(95vh, calc(var(--vh, 1vh) * 95));">
      <div class="p-3 sm:p-6 border-b border-theme-border flex items-center justify-between bg-gradient-to-r from-[var(--theme-gradient-start)]/10 to-[var(--theme-gradient-end)]/10">
        <h2 class="text-base sm:text-xl font-bold gradient-text flex items-center gap-2">
          ...
          <span class="hidden sm:inline">自定义模型配置</span>
          <span class="sm:hidden">模型配置</span>
        </h2>
      ...
      <div class="flex flex-1 overflow-hidden flex-col sm:flex-row">
            ...
            >
              <div class="flex items-center justify-between">
                <span class="font-medium text-theme-text-primary truncate">
                  {{ config.name || config.default_model || '未命名配置' }}
                </span>
                <div class="flex items-center gap-1">
                  <span v-if="config.is_default" class="text-xs px-2 py-0.5 bg-[var(--theme-success)]/20 text-[var(--theme-success)] rounded-full">默认</span>
                  <span v-if="modelConfigStore.activeConfigId === config.id" class="text-xs px-2 py-0.5 bg-[var(--theme-primary)]/20 text-[var(--theme-primary)] rounded-full">使用中</span>
                </div>
              </div>
              <div class="text-xs text-theme-text-secondary mt-1 truncate">
                {{ config.provider }} · {{ config.api_url || '未配置' }}
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <template v-if="selectedConfig">
            <!-- 配置名称 -->
            <div>
              <label class="block text-sm font-medium text-theme-text-primary mb-2">配置名称</label>
              <input
                type="text"
            ...
            <!-- 提供商 -->
            <div>
              <label class="block text-sm font-medium text-theme-text-primary mb-2">提供商</label>
              <select
            ...
            <!-- API 地址 -->
            <div>
              <label class="block text-sm font-medium text-theme-text-primary mb-2">API 地址</label>
              <input
                type="text"
            ...
            <!-- API Key -->
            <div>
              <label class="block text-sm font-medium text-theme-text-primary mb-2">API Key</label>
              <input
                type="password"
            ...
            <!-- 模型选择 -->
            <div>
              <label class="block text-sm font-medium text-theme-text-primary mb-2">模型</label>
